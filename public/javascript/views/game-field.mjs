...
export const startTimer = async (socket, duration, index, gameDuration) => {
    pressedKeys = [];
    ...
    let coundDown = setInterval(() => {
        ...
        if (--timer < 0) {
            ...
            gameDurationTimer(socket, gameDuration);
...
const createUserForGame = (userName, userId, ready) => {
    ...
    const status = createElement({
        tagName: 'div',
        className: `${ready ? 'user__status user__status-active' : 'user__status'}`,
        attributes: {
            'data-user-id': userId
        }
    });
